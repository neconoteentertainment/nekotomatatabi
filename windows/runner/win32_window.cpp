...
void EnableFullDpiSupportIfAvailable(HWND hwnd) {
  ...
  if (enable_non_client_dpi_scaling != nullptr) enable_non_client_dpi_scaling(hwnd);
...
class WindowClassRegistrar {
 public:
  ...
  static WindowClassRegistrar* GetInstance() { if (!instance_) instance_ = new WindowClassRegistrar(); return instance_; }
...
LRESULT Win32Window::MessageHandler(HWND hwnd, UINT const message, WPARAM const wparam, LPARAM const lparam) noexcept {
  switch (message) {
    ...
    case WM_SIZE: {
      ...
      if (child_content_ != nullptr) MoveWindow(child_content_, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, TRUE);
      return 0;
    }
    case WM_ACTIVATE:
